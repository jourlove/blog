...
<div class="result_wrap">
    <form action="{{url('admin/web_config/'.$field->id)}})}}" method="post">
        {{method_field('PUT')}}
        {{csrf_field()}}
        <table class="add_tab">
            <tbody>
            ...
            <tr>
                <th><i class="require">*</i>变量名称：</th>
                <td>
                    <input type="text" name="var_name" value="{{$field->var_name}}">
            ...
            <tr>
                <th>类型：</th>
                <td>
                    <input type="radio" name="content_type" value="input" @if($field->content_type=='input') checked @endif onclick="showTr()">input　
                    <input type="radio" name="content_type" value="textarea" @if($field->content_type=='textarea') checked @endif onclick="showTr()">textarea　
                    <input type="radio" name="content_type" value="radio" @if($field->content_type=='radio') checked @endif onclick="showTr()">radio
                </td>
            </tr>
            <tr class="content_value">
                <th>类型值：</th>
                <td>
                    <input type="text" class="lg" name="content_value" value="{{$field->content_value}}">
